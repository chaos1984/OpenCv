...
@ray.remote
def frameprocess(bg,pyrDownNum,frame,t0,frame_id):
    ...
    contours_list, hierarchy = cv2.findContours(thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
...
class CushionTracking():
    ...
    def multiProccessing(self,video_file):
        ...
        while(True):
            ...
            last_frame_time = t
            # print ("frame_id",frame_id  )
            res = frameprocess.remote(self.bg,self.pyrDownNum,frame,0,frame_id)
            contours_list = ray.get(res)
                    
            # res_list.append(res)
            res = self.getROI(frame_id,frame,contours_list)
    ...
    def getROI(self,frame_id,frame,contours_list):
        areas = []
        index_list =[]

        for index,c in enumerate(contours_list):
            ...
            try:
                ...
                contourArea =cv2.contourArea(contours_list[index])
